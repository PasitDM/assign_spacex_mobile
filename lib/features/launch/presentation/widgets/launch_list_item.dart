...
import '../../../../common/widgets/images/app_image.dart';
...
class LaunchListItem extends StatelessWidget {
    ...
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Responsive(
            mobile: _buildMobileLayout(context, launch, launchDate),
            tablet: _buildTabletLayout(context, launch, launchDate),
          ),
        ),
      ),
    );
  ...
  Widget _buildMobileLayout(BuildContext context, Launch launch, DateTime launchDate) {
    return Row(
  ...
  Widget _buildTabletLayout(BuildContext context, Launch launch, DateTime launchDate) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(launch.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.black)),
              ...
              Text(
                'Result: ${launch.success ? 'Success' : 'Failure'}',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: launch.success ? Colors.green : Colors.red),
              ),
            ],
          ),
        ),
        ...
        Builder(
          builder: (context) {
            if (launch.patchSmall.isNotEmpty) {
              return ClipRRect(
                borderRadius: BorderRadius.circular(12),
                child: AppImage(imageUrl: launch.patchSmall, width: 100, height: 100),
              );
            } else {
              return Icon(Icons.error, color: Colors.grey[400], size: 100);
            }
          },
        ),
      ],
